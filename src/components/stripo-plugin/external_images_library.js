window.ExternalImagesLibrary = (function() {
    ...
    var cancelCallback;
  let externalImageUrlInput;
  let externalImageUrl = '';
  let submitExternalImageUrlButton;
    ...
    var initLibrary = function() {
        ...
        div.innerHTML = '\
            ...
            <input id="externalImageUrlInput" style="margin: 10px; width: calc(100% - 100px)" placeholder="External Image Url">\
            <button id="externalImageUrlButton" class="image-submit-button ml-20 rounded py-0 px-16 min-h-10 w-[100px]">OK</button>\
            </div>\
      ...
      externalImageUrlInput = document.getElementById(
        'externalImageUrlInput',
      );

      submitExternalImageUrlButton = document.getElementById(
        'externalImageUrlButton',
      );

      externalImageUrlInput.addEventListener('input', (event) => {
        externalImageUrl = event.target['value'];

        if (
          !externalImageUrl &&
          !submitExternalImageUrlButton.hasAttribute('disabled')
        ) {
          submitExternalImageUrlButton.setAttribute('disabled', '');
        } else if (
          externalImageUrl &&
          submitExternalImageUrlButton.hasAttribute('disabled')
        ) {
          submitExternalImageUrlButton.removeAttribute('disabled');
        }
      });

      submitExternalImageUrlButton.addEventListener(
        'click',
        (event) => {
          if (externalImageUrl) {
            imageSelectCallback({ url: externalImageUrl });
            close();
          }
        },
      );
